...
constants.define(exports, {
    FETCH_CITY_WISE_DATA: "SELECT SUM(trip.trip_price) as total_revenue,YEAR(date(trip.trip_approved_time)) as trip_year FROM trip WHERE trip.property_id IN (?) and trip.trip_status = 'ACCEPTED' GROUP BY trip.property_id , trip_year ORDER BY trip_year limit ?;",
